...
void printLevelWise(TreeNode<int>* root) {
    ...
    queue<TreeNode<int>*>forPrinting;
    forPrinting.push(root);

    while(forPrinting.size()!=0){
        TreeNode<int>*newPrint = forPrinting.front();
        forPrinting.pop();
        cout<<newPrint->data<<":";
        for(int i =0; i<newPrint->children.size(); i++){
            if(i!=newPrint->children.size()-1){
                cout<<newPrint->children[i]->data<<",";
                forPrinting.push(newPrint->children[i]);
            }
            else{
                cout<<newPrint->children[i]->data;
                forPrinting.push(newPrint->children[i]);
