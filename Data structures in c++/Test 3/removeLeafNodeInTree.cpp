...
TreeNode<int>* removeLeafNodes(TreeNode<int>* root) {
	...
	// here Tree node is a type of dynamic vector
	if(root == NULL || root->numChildren()==0) return NULL;

	vector<int> indexes;
	// we are saving the indexes of the root whose child = 0;
	...
	// see remove child function, since j+1==childcount and we are doing it from the back, we are just doint childcount--;
	for(int i = indexes.size()-1; i>=0; i--){
		int j = indexes[i];
		// then we are removing the child with the given function
		root->removeChild(j);
	}

	for(int i = 0; i<root->numChildren(); i++){
		// and then recursively calling the function again for the root's children
