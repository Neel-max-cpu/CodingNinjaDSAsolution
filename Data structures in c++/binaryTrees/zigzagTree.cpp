...
void zigZagOrder(BinaryTreeNode<int> *root) {
    ...
    while (pendingNodes.size() != 0)
    {
		...
		// if level is even print right to left
        if (level % 2 == 0)
        {
            for (int i = n - 1; i >= 0; i--)
                cout << pendingNodes[i]->data << " ";
        }
		// if level is odd print left to right
        else
        {
		...
		// erase function erase eg = (l,r), would erase the elemets from l to r-1, so giving the size it would delete elements from 0 to n-1
		// eg if we want to delete the 3 elements from 1st then v.erase(v.begin(), v.begin()+3+1) or  v.erase(v.begin(), v.begin()+4)
